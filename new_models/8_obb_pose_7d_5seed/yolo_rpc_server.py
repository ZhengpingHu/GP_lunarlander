...
# -------------------
# 7D 状态估计器（线程安全 predict + 单实例 YOLO）
# -------------------
class YoloStateEstimator7D:
    def __init__(self,
        ...
        # [FIX] 线程锁（默认新建；clone 时会被替换为共享锁）
        self._lock_obb = Lock()
        self._lock_pose = Lock()

        # [FIX] 保存必要的初始化参数，避免把锁塞进去
        ...
        # [FIX] warmup：触发一次 fuse，避免并发时重复 fuse 引发 AttributeError: bn
        if not _skip_warmup:
            try:
    ...
    def clone(self) -> 'YoloStateEstimator7D':
        """
        创建一个新的估计器实例，但共享已加载的YOLO模型与预测锁。
        这样可以为每个客户端会话提供独立的状态，同时避免重复加载模型的开销，并保证 predict 线程安全。
        """
        args = dict(self._init_args)  # 仅必要参数
        # 传入已加载的模型对象，而不是路径；并跳过 warmup
...
def main():
    ...
    # 1. 创建一个“母版”估计器实例，它会加载模型到GPU并 warmup
    ...
    # 2. 将母版实例传给服务器。服务器会为每个客户端“克隆”它（共享模型与锁）
